...
*   **Hardware Keyboard**: **Game Controller Framework (`GCKeyboard` / `GCController`)**.
    *   *Why*: Standard `UITextField` input has latency (>30ms) and focus issues. `GCKeyboard` provides <8ms polling for "PRO" input handling.
*   **On-Screen Keyboard**: **Custom SwiftUI Views**.
    *   Visual aid only ("Ghost Keyboard").

### 2.5 Audio & Haptics Engine
*   **Audio**: **AVAudioEngine**.
    *   Two-channel mixer (SFX/Ambience). Low latency buffers.
*   **Haptics**: **UIImpactFeedbackGenerator**.
    *   Used for "Perfect Word" streaks and "Level Up" events.

## 3. Real-Time Data Binding
*   **Primary**: **Swift Concurrency (`async/await`)** + **`@Observable`** (iOS 17+).
*   **Combine**: *Legacy Support*. Can be used for specific high-frequency event streams if `AsyncStream` proves insufficient, but `@Observable` is preferred for UI binding.
