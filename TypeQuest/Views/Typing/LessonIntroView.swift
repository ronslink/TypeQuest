            ...
            VStack(spacing: 0) {
                ScrollView {
                    VStack(spacing: 30) {
                        ...
                        Spacer(minLength: 120) // Scroll padding
                    }
                    .padding(.vertical, 40)
                    .padding(.horizontal)
                }
...
struct LessonKeyboardView: View {
    ...
    private func isKeyHighlighted(_ key: String) -> Bool {
        // Simple deduplication from activeKeys
        let activeChars = activeKeys.flatMap { simpleKeyMap($0) }
        return activeChars.contains(key.lowercased())
    }
    
    private func simpleKeyMap(_ key: AbstractKey) -> [String] {
        switch key {
        case .homeLeftPinky: return ["a"]
        case .homeLeftRing: return ["s"]
        case .homeLeftMiddle: return ["d"]
        case .homeLeftIndex: return ["f", "g"]
        case .homeRightIndex: return ["h", "j"]
        ...
        case .topLeftIndex: return ["r", "t"]
        case .topRightIndex: return ["y", "u"]
        ...
        case .bottomLeftIndex: return ["v", "b"]
        case .bottomRightIndex: return ["n", "m"]
    ...
    private func fingerColor(for key: String) -> Color {
        // Return color based on which finger SHOULD press this key
        // Left Pinky: Q A Z
        let k = key.lowercased()
        
        if isHomeRowMode, !isKeyHighlighted(key) {
             return .gray.opacity(0.2) // Dim others in home row mode
        }
        
        if ["`", "1", "q", "a", "z"].contains(k) { return .pink } // L Pinky
        if ["2", "w", "s", "x"].contains(k) { return .orange } // L Ring
        if ["3", "e", "d", "c"].contains(k) { return .yellow } // L Middle
        if ["4", "5", "r", "t", "f", "g", "v", "b"].contains(k) { return .green } // L Index
        
        if ["6", "7", "y", "u", "h", "j", "n", "m"].contains(k) { return .cyan } // R Index
        if ["8", "i", "k", ","].contains(k) { return .blue } // R Middle
        if ["9", "o", "l", "."].contains(k) { return .purple } // R Ring
        if ["0", "-", "=", "p", "[", "]", "\\", ";", "'", "/", "⌫", "↩", "⇧"].contains(k) { return .red } // R Pinky
        
        if k == " " { return .gray } // Thumb
...
struct LessonKeyView: View {
    ...
    var body: some View {
        ...
        .frame(width: keyWidth(for: key), height: 32) // Slightly smaller
